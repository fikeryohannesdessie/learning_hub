...
public class FlutterAnglePlugin implements FlutterPlugin, MethodCallHandler {
  ...
  private static boolean angleLibrariesLoaded = false;
  ...
  private void initOpenGLAngleImplementation(MethodChannel.Result result) {
    if (!angleLibrariesLoaded) {
      Log.w(TAG, "ANGLE native libraries are unavailable, falling back to standard OpenGL");
      initOpenGLImplementation(result);
      return;
    }

    // Plugin2: using native ANGLE functions
    final boolean initResult;
    try {
      initResult = init();
    } catch (UnsatisfiedLinkError e) {
      Log.e(TAG, "ANGLE JNI init symbol is unavailable, falling back to standard OpenGL", e);
      initOpenGLImplementation(result);
      return;
    }
  ...
  private void createTextureAngleImplementation(MethodCall call, MethodChannel.Result result) {
    if (!angleLibrariesLoaded) {
      Log.w(TAG, "ANGLE texture path is unavailable, falling back to standard OpenGL texture creation");
      createTextureImplementation(call, result);
      return;
    }
